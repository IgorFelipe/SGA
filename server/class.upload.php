...
//exit;
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        $this->translation['incorrect_file']              = '<b style="font-size: 30px;">O tipo de arquivo que se esta tentando enviar nao esta correto. Tente Novamente...</b><br/><br><a href="javascript:window.history.go(-1)" style="font-size: 30px">VOLTAR NO ENVIO</a>';
...
//		echo ' -> '.$this->mime_check .' -- '.empty($this->file_src_mime).' - '.strpos($this->file_src_mime, '/').'<br>';
...
//echo 'antes---------------'.$this->file_src_mime;

                foreach($this->forbidden as $k => $v) {
				//echo ' k '.$v. ' ** '.$this->forbidden[$k];
                    list($v1, $v2) = explode('/', $v);
                    if (($v1 == '*' && $v2 == '*') || ($v1 == $m1 && ($v2 == $m2 || $v2 == '*'))) {
                        $allowed = false;
                        break;
...
//			echo 'depoios -->'.$this->file_src_mime;
